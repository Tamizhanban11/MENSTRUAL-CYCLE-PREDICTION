...
from chatbot import chatbot_ui  # Import the chatbot UI function
...
def signup():
    ...
    if st.button("Sign Up"):
        if username in users:
            ...
            st.rerun()  # Rerun the app after signup
...
def home():
    ...
    if not os.path.exists(DATA_FILE):
        ...
        try:
            ...
            xgb_model.load_model("models/xgboost_model.json")  # Ensure correct path
        ...
        if current_period_date <= previous_period_date:
            ...
            if st.button("🔮 Predict Next Two Periods"):
                try:
                    ...
                    dmatrix_input = xgb.DMatrix(np.array([[cycle_length, pain_level, fatigue_level, mood_swings, nausea]]))
                    xgb_pred = xgb_model.predict(dmatrix_input)
...
def main():
    ...
    app_selection = st.sidebar.radio("Select an App", ["Home", "Chatbot"])

    if app_selection == "Home":
        st.write("Welcome to the main app!")
        home()  # Call the home function to display the menstrual cycle prediction app UI
    elif app_selection == "Chatbot":
        chatbot_ui()  # Call the chatbot UI function
